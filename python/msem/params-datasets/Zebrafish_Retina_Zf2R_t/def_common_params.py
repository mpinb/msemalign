...
print('def_common_params: Zebrafish_Retina_Zf2R_t - 2 wafers')
...
from def_common_params_common import init_region_info, cimport_exclude_regions
...
# to allow for the discontiguous / partially image sections (multiple section regions).
# in this case some mfovs are not imaged (so mfov directories and/or tile images are absent).
allow_empty_mfovs = False
...
total_nwafers = 2 # change this appropriately for each dataset
...
experiment_subfolder = '2025/Zebrafish_Retina_Zf2R_t'
...
meta_subdirs = ['debug_plots', 'debug_plots/sift', 'debug_plots/outliers', 'order_plots', 'region_plots', 'webknossos']
...
raw_folders_all = [
        ...
        ['Zebrafish_Retina_Zf2R_t_W1',],  # 4059
        ['Zebrafish_Retina_Zf2R_t_W2',],  # 3997
        ]
...
manifest_suffixes = [None] + ['_manifest.txt']*total_nwafers
...
reimage_beg_inds = [None, # no wafer 0, leave blank
                    [4060,], # wafer 1
                    [3998,], # wafer 2
                   ]
...
#exclude_regions = [[] for x in range(total_nwafers+1)] # for not excluding any additional regions
exclude_regions = import_exclude_regions() # rough alignment iterations 1-2, after order solving

region_strs_all = [None]*(total_nwafers + 1)
region_rotations_all = [None]*(total_nwafers + 1)
region_reimage_index = [None]*(total_nwafers + 1)
region_timestamps_all = [None]*(total_nwafers + 1)
...
init_region_info(all_wafer_ids, root_raw, get_paths, raw_folders_all, alignment_folders,
    ...
    region_strs_all, region_rotations_all, region_reimage_index, region_timestamps_all,
    region_manifest_cnts, region_include_cnts, exclude_regions)
...
# !!! IMPORTANT: rerun wafer-init if you modify these becuase they are stored in the limi dill !!!
...
czifile_rotations = [None, 1.83075, -0.38201538] # zebrafish retina
...
# series of fine alignment croppings to use. the purpose is to start at small crop values and run larger ones
...
#crops_um = [[32., 32.],] # fine alignment iteration 1
#crops_um = [[24., 24.],] # fine alignment iteration 2
#crops_um = [[16., 16.],] # fine alignment iteration 3
crops_um = [[13.6, 13.6],] # ultrafine alignment
...
slice_blur_factor = 5.5 # factor that increases the LOG sigma value when pre-procing image / template
...
# C_cutoff_soft_nGMM == 5 # # 2D alignment iteration 1
# C_cutoff_soft_nGMM == 0 # # 2D alignment iteration 2
C_cutoff_soft_nGMM = 0 # disable the soft cutoff
...
weight_default = [[0.1, 0.001, 1.], [0.1, 0.001, 1.], [0.1, 0.001, 1.]]
...
# "gratings" in the epon (bad chatter creating non-separated ribboning) require a tigher tolerance
#   on the between mfov tolerances. this also results in bad median estimates, necessitating
#   a looser within mfov tolerance.
# for datasets with difficult epon, require two full 2D alignment iterations:
#   (1) 2D align (with errors) to create the masks and (2) 2D align again using masks to reject non-tissue tiles
# these are commented as "2D alignment iteration 1" and "2D alignment iteration 2" respectively
...
#region_stitching_twopass_nregions = 21 # 2D alignment iteration 1
region_stitching_twopass_nregions = 51 # 2D alignment iteration 2
...
#twopass_default_tol_nm = [[96,96], [1024,1024]] # 2D alignment iteration 1 (without masks)
twopass_default_tol_nm = [[96,96], [4480,4480]] # 2D alignment iteration 2 (with masks)
...
mode_ratio_block_shape_um = (0.976, 0.976)
...
# this is most likely not necessary for large slices, particularly datasets with good cutting / contrast.
...
brightness_balancing_chunksize = 5500
...
brightness_balance_slices_whole_tiles_nslices = 32
...
# BUT run brightness balancing with masks after order solved for final sliding histogram matching
brightness_balancing_slices_maxlag = 96
...
#def_overlap_correction_borders = [0,0,0,0] # for no correction
#def_overlap_correction_borders = [1,1,1,1] # to correct all
def_overlap_correction_borders = [0,1,1,1] # do not correct top
...
# xxx - after some time noticed that these overlap numbers are not the same between sections in the same dataset
#   just use a consistent value unless this somehow changes dramatically for some future dataset.
#   stick with an feathering distance that is probably > 1.5x the actual overlap, generally looks better?
blending_mode_feathering_min_overlap_dist_um = [0.624, 0.72]
...
# NOTE: indexed by wafer_id (not index)
# NOTE: with the new region_str system, these are 1-based region indices, NOT slice numbers
# NOTE: on initial runs before tears are actually stitched, keep this as None.
#   otherwise region and wafer code will try to load the stitched region.
#torn_regions = None # to disable tear fixing in individual slices / regions
torn_regions = [[], # no wafer 0, leave blank
    [425, 543, 708, 922, 2331, 3742], # 6 w1
    [], # wafer 2
    ] # 6 total
...
# this is for defining polygons that outline the tissue in datasets tha need them for alignment.
# xxx - this solution did not work as is, the static polygon per wafer is not accurate enough.
#   decided to leave the code hooks in for now in case this ever gets revived... meh
use_tissue_coordinates = False
# for datasets where these are defined statically per wafer, define a translation correction factor
#   for the tissue coordinates relative to the roi coordinates. this can easily correct for
#   issues that occurred with this file during acquisition.
# None for any wafer means no correction factor and use the tissue polygon as it is saved.
tissue_coordinates_centroid = [None for x in range(total_nwafers+1)]
#tissue_coordinates_centroid = [None, [245.31276095, 6626.59918101]]
...
#delta_rotation_range = [-20.,20.] # fine alignment iterations 1-2
#delta_rotation_range = [-12.,12.] # fine alignment iteration 3
delta_rotation_range = [-9.,9.] # ultrafine alignment

# degree steps for delta template match
#delta_rotation_step = 4 # fine alignment iterations all
delta_rotation_step = 3 # ultrafine alignment
...
#template_crop_um=[4.8,4.8] # fine alignment iterations all
template_crop_um=[3.2,3.2] # ultrafine alignment
...
roi_polygon_scale = 1.2 # used with fine alignment
...
rough_bounding_box_xy_spc = [25, 42, 24]

# these are for the parameters of the hex grid to define the rough alignment grid.
# parameters are number of x points, number of y points and point spacing in microns.
rough_grid_xy_spc = rough_bounding_box_xy_spc

# these are for the parameters of the hex grid to define the fine alignment grid.
# parameters are number of x points, number of y points and point spacing in microns.
#fine_grid_xy_spc = [35, 60, 16] # fine alignment iteration 1, minimum-sized fine grid
#fine_grid_xy_spc = [47, 80, 12] # fine alignment iteration 2
#fine_grid_xy_spc = [72, 121, 8] # fine alignment iteration 3
fine_grid_xy_spc = [244, 461, 2] # ultrafine alignment, minimum-size grid
...
tissue_mask_fn_str = 'z_{:06d}.tiff' # from webknossos dataset indexed by z, webknossos export-wkw-as-tiff
tissue_mask_ds = 32 # 2D alignment iterations 1,2, rough alignment iteration 1, region up_masks
#tissue_mask_ds = 1 # after upsampling masks, for rough alignment iteration 2, fine alignment iterations all
# these are only applied when using the masks (not when saving)
#   EXCEPT for the upsampling, it stores the upsampled masks including the manipulations.
# run upsample after rough alignment iteration 1
# ONLY when upsampling masks == region up_masks
#tissue_mask_min_edge_um = 24
#tissue_mask_min_hole_edge_um = 24
#tissue_mask_bwdist_um = 24
# for no manipluation
# 2D alignment iterations all except upsample masks, rough alignment iterations all, fine alignment iterations all
# region save_masks
...
# set "nfeatures" in sift_args to limit the max number of sift features to use
#   (number specified to opencv, it returns them sorted by "prominence").
# NOTE: unlimited features really needs faiss-gpu knn_search_method (not exposed).
# ordering solving, use opencv defaults (less features, still works well)
#sift_args = {} # for unlimited features and opencv defaults
#sift_args = {'nfeatures':10000}
# chatGPT suggested based on prompt:
# "Which opencv SIFT parameters would be more optimal for matching other images
#  versus the default for low contrast 8-bit grayscale images?"
# rough alignment iteration 1, set sift_args to get more matching features for low tissue area sections
sift_args = {
    "nOctaveLayers": 4,
...
# affine_rigid_type == 3, (!) rough alignment iteration 1, sift nonuniform
# NOTE: need to change this before running fine_to_rough
# affine_rigid_type == 3, rough alignment iteration 2, fine to rough
...
#min_feature_matches = 32 # recommend slightly larger but still low whilst order solving
# rough alignment iteration 1,2
min_feature_matches = 128
# could set even higher (128 or 256) to try to use rough alignment to vet bad matches

# for the thumbnail affine fitting (ransac parameter)
# IMPORTANT: do NOT use a different value for percent matches with ransac,
rough_residual_threshold_um = 5.12 # sift nonuniform
...
max_fit_translation_um = [256.,256.]
...
keypoints_nworkers_per_process = 12
...
# set this to 2.5x the rough grid spacing.
#rough_distance_cutoff_um = 60 # rough alignment iteration 1, sift nonuniform
rough_distance_cutoff_um = 0 # rough alignment iteration 2, fine to rough
...
# NOTE: same parameters are used for rough alignment iteration 2 (fine-to-rough) as for fine alignment iteration 1,
...
#interp_inlier_nneighbors = 817 # hex grid of diameter 33
# fine alignment iterations 2-3
#interp_inlier_nneighbors = 1027 # hex grid of diameter 37
# ultrafine alignment
interp_inlier_nneighbors = 1261 # hex grid of diameter 41
...
fine_interp_neighbor_dist_scale = [1., 1., 0.4, 0.2, 0.1]
...
# NOTE: use sqrt(3)/2 to compute y distance, do NOT use the make_hex code because the bounding box
#   is never centered on any center point, so this will be misleading.
#fine_filtering_shape_um = [82., 84.9] # fine alignment iteration 1, 5.125 x 6.125 hex grid at 16 spacing
#fine_filtering_shape_um = [49.5, 53.26] # fine alignment iteration 2, 4.125 x 5.125 hex grid at 12 spacing
#fine_filtering_shape_um = [33., 35.5] # fine alignment iteration 3, 4.125 x 5.125 hex grid at 8 spacing
# ultrafine alignment
fine_filtering_shape_um = [0., 0.] # fine filtering step disabled, so value does not matter

# whether to remove the bias from the solver by using linear regression (instead of mean).
# keep both True, have not found a case where this hurts and usually helps significantly.
rough_regression_remove_bias = True
fine_regression_remove_bias = True
...
# rough alignment iteration 1, need to decide on a case-by-case basis to enable or not
#   for multi-wafer datasets maybe disable to balance offsets between wafers
#   typically always enable for rough alignment iteration 1 for single-wafer datasets
#z_neighbors_radius = [0,0] # to disable, solve each grid point using all slice and neighbor deltas
# rough alignment iterations 1-2, fine alignment iterations all
z_neighbors_radius = [252, 84] # == 505 neighboring slices, about 20 um at 40 nm slice thickness
...
# NOTE: you have tried going to 3 and higher degrees many times and outlier detection is always worse.
outlier_affine_degree = 2

# residual threshold for the ransac affine fit
#fine_residual_threshold_um = 16. # fine alignment iterations 1-2
#fine_residual_threshold_um = 12. # fine alignment iteration 3
fine_residual_threshold_um = 2. # ultrafine alignment
...
#ninlier_neighhbors_cmp = [-8, 37] # fine alignment iterations 1-2
#ninlier_neighhbors_cmp = [-9, 61] # fine alignment iteration 3
ninlier_neighhbors_cmp = [-11, 91] # ultrafine alignment
...
#ok_outlier_zscore = 0. # to disable ok outliers
#ok_outlier_zscore = -12. # fine alignment iterations 1-2
#ok_outlier_zscore = -6. # fine alignment iteration 3
ok_outlier_zscore = -0.64 # ultrafine alignment
...
#not_ok_inlier_zscore = -4. # fine alignment iteration 1
#not_ok_inlier_zscore = -2. # fine alignment iteration 2
#not_ok_inlier_zscore = -1 # fine alignment iteration 3
not_ok_inlier_zscore = -0.16 # ultrafine alignment
...
#merge_inliers_blk_cutoff = 25 # fine alignment iterations 1-2
#merge_inliers_blk_cutoff = 19 # fine alignment iteration 3
merge_inliers_blk_cutoff = 15 # ultrafine alignment
